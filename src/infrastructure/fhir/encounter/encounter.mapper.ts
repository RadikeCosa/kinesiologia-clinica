...
const NOTE_FIELDS = ["subjective", "objective", "intervention", "assessment", "tolerance", "homeInstructions", "nextPlan"] as const;
const SUFFIX = { subjective: "subjective", objective: "objective", intervention: "intervention", assessment: "assessment", tolerance: "tolerance", homeInstructions: "home-instructions", nextPlan: "next-plan" };
const LEGACY = { subjective: "clinical-subjective:v1:", objective: "clinical-objective:v1:", intervention: "clinical-intervention:v1:", assessment: "clinical-assessment:v1:", tolerance: "clinical-tolerance:v1:", homeInstructions: "clinical-home-instructions:v1:", nextPlan: "clinical-next-plan:v1:" };
...
export function mapFhirEncounter(resource: FhirEncounter): Visit {
  ...
  for (const field of NOTE_FIELDS) {
    ...
    const legacy = resource.note?.find((item) => item.text?.startsWith(LEGACY[field]))?.text?.slice(LEGACY[field].length);
  ...
  return {
    ...
    status: resource.status === "finished" ? "finished" : "in-progress",
